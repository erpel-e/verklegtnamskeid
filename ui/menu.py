...
class Menu:
	...
	def authenticate(self):
		"""This method check the authentication of the employee, by entering the 
		username and password, and it will loop until it has the correct
	...
	def signout(self):
		"""In case that the employee signs out, it prints sign out and calls the 
		authenticate() method to wait for the employee to sign in again.
		"""
		self.frame.delete_last_lines(9)
		employee = self.employee_manager.get_current_employee() 
		# Prints that the user has been logged out
		print("{} has been logged out".format(  				
			self.color.return_colored(employee.get_name(), "red")))
		# Lets the program sleep for 1.5 seconds before continuing for a regular flow.
		time.sleep(1.5)  
		self.frame.delete_last_lines(3)
		self.authenticate()

	def check_if_valid(self, to_enter, to_check, editing=False, current_value=""):
		"""Controls the output for most of our error checks and edit methods."""
		mistake = 0
		error = "check if valid"
		while error:
			if (not editing and not current_value) or (
				not current_value and type(editing) == list):
				user_input = input("Enter " + to_enter + ": ")
			else:
				if to_enter == "Credit card number":
					# When editing a credit card number block out the first 12 numbers
					user_input = input(
						"Enter " + to_enter + " [**** **** **** " + current_value[12:] + "]: ")
				else:
			...
			if editing and current_value and not error:
				...
				if user_input != "":
					...
					if to_enter == "Credit card number":  
						print("Enter " + to_enter + " [**** **** **** " + current_value[12:] 
							+ "]: " + "**** **** **** " + current_value[12:])
					else:
	...
	def invalid_input(self, message):
		...
		provides the correct error message to suggest why the inputed data is wrong.
		"""
	...
	def init_menu(self):
		"""Initial menu/Homepage. The employee can choose any of the operations 
		and continue with the functionalities of the page, or he/she can sign out.
		"""
	...
	def report_error(self):
		"""This method prints how to reach help for any problems encountered while
		using this service, and then go back at the main page.
		"""
		self.frame.delete_last_lines(7)
	...
	def customer(self):
		"""Prints functionalities of the customer menu, and the user can either choose 
		any option from the menu, either can go back at main menu.
		"""
	...
	def order(self):
		"""Prints functionalities of the order menu, and the user can either choose 
		any option from the menu, either can go back at main menu.
		"""
	...
	def vehicles(self):
		"""This function allows the user to choose what action he wants after going 
		to the section "cars" using nocco list"""
		self.frame.delete_last_lines(7)
	...
	def handle_answer_from_order_menus(self, prompt, menu_type):
		...
		####################################################
			...
			if prompt == "Save":
				...
				self.frame.delete_last_lines()
				try:
					temp_order = self.order_manager.find_order_by_id(
						self.__current_order.get_id())
					if not temp_order:
						raise ValueError
					self.__current_order = temp_order
				except ValueError:
					temp_order = self.order_manager.find_order_by_ssn(
						self.__current_order.get_ssn())
					if temp_order:
	...
	def calculate_order(self):
		"""After the employee has inputed all the neccessary data, it calculates 
		the order and prints it in a readable style, with correct formating.
		"""
	...
	def show_pricing_list(self):
		"""This simple method prints the pricing list, in case that the customer wants 
		to know what kind of car he can afford, and what the price ranges are.
		"""
		vehicle_types = ["smallcar", "sedan", "offroad", "bus"]  # Available types we offer.
		self.frame.delete_last_lines(2)
		print()
		print("{:<15}{:>12}{:>20}{:>20}".format(
			"Vehicle type", "Base price", "Basic insurance", "Extra insurance"))
		print("-"*67)
		# Loops the vehicle list and prints the price for each vehicle.
	...
	def find_order(self):
		"""This method provides the options of the Find order in Order menu. """
	...
	def find_order_by_id(self):
		"""This method makes possible for the employee to find any order in the 
		database (csv) with the id of the order. After the order is found, it 
		provides further options to continue.
		"""
		ID = input("Enter ID: ")
		self.frame.delete_last_lines(2)
		print()
		order = self.order_manager.find_order_by_id(ID)  # Searching for the order
	...
	def find_order_by_ssn(self):
		"""This method makes possible for the employee to find any order in the 
		database (csv) with the SSN of the customer. After the order is found, it
		provides further options to continue.
		"""
		ssn = input("Enter SSN: ")
		print()
		orders = self.order_manager.find_order_by_ssn(ssn)  # Searching for orders
		# Than the order has not been found because there is no order linked to that ssn
		if orders == []:  
			print("{}".format(self.color.return_colored("Order not found!", "red")))
			time.sleep(1.5)
			self.frame.delete_last_lines(3)
			...
			if len(orders) == 1:  # There is only one order linked to that particular ssn
				...
				print("{}".format(self.color.return_colored(
					"There are multiple orders with that SSN!", "red")))
				print()
				# Create a list of all the orders found with order IDs and dates
				...
				# Using that list in a menu so that employees can pick one
				...
				self.handle_answer_from_order_menus(
					(found_multiple_orders, orders), "found multiple orders")

	def found_order(self):
		"""After the employee has found the order that was looking for, this 
		method provides the functionalitiesof what the employee can do 
	...
	def get_inputted_order(self):
		"""Prints out the information of a previously entered order, then a menu 
		will appear with the functionalities to move on in further operations.
		"""
	...
	def delete_order(self):
		...
		# Getting the dates and license plate for the order
		...
		self.vehicle_manager.delete_order_dates(dates, vehicle) # Deleting dates from vehicle
	...
	def save_new_order(self):
		...
		time.sleep(1.5)
		# Getting the dates of the order and saving them to the correct vehicle
	...
	def register_order(self):
		"""When the employee wants to register a new order, this method is called and 
		enables the employee to enter the correct information, by calling the 
		correct checks for each data entered. After the employee has entered all the 
		neccessary data, then another sub menu is provided to allow the customer 
		to pick a car in a range of available cars. After the customer has chosen 
		the car, then the employee can proceed in further options.
		"""
		self.frame.delete_last_lines(6)
		print()
		# Here are all checks to check if a input is valid, it is sent to the 
		# check_if_valid function and order_manager checks.
		...
		self.check_if_valid(
			"pick up location (Reykjavik or Akureyri)", 
			self.order_manager.check_pick_up_location)
		self.check_if_valid(
			"return location (Reykjavik or Akureyri)", 
			self.order_manager.check_return_location)		
		self.check_if_valid(
			"type of vehicle (Small car, sedan, offroad or bus)", 
			self.order_manager.check_type_of_vehicle)
		
		start_date, end_date = self.order_manager.get_dates()
		vehicle_list = self.vehicle_manager.show_vehicle_availability(
		...
		if not filtered_list:  # In case that there are not any available car. 
			self.frame.delete_last_lines(1)
			print()
			print("No vehicle of type {} available on these dates.".format(
				self.color.return_colored(vehicle_type, "red")))
			time.sleep(1.5)
			self.frame.delete_last_lines(11)
			self.order()
		else:  # Otherwise it will display all the cars available.
			plates = []
	...
	def edit_order(self):
		"""This method is called to edit any order. The employee has the ability 
		to change or leave as it is any data. Any data that wants to be changed 
		can be done by entering the new data and then hit enter, otherwise if the 
		employee wants to keep the previous data can skip editing by leaving the 
		input field empty and hit enter. For each data, it is called check method 
		to check if the data entered is correct, or in correct format.
		"""
		order = self.__current_order.return_details()
		print("{}\n".format(self.color.return_colored(
			"Leave input empty to keep the value the same.", "green")))
		# Checks if inputs are valid, goes thorugh check_if_valid function and 
		# checks in order_manager.
		...
		self.check_if_valid(
			"start date", self.order_manager.check_start_date, True, order["Start date"])
		self.check_if_valid(
			"end date", self.order_manager.check_ending_date, True, order["End date"])
		self.check_if_valid(
			"pick up time", 
			self.order_manager.check_pick_up_time, 
			True, 
			order["Pick up time"]
		)
		self.check_if_valid(
			"return time", 
			self.order_manager.check_returning_time, 
			True, 
			order["Return time"]
		)
		self.check_if_valid(
		...
		self.check_if_valid(
			"type", self.order_manager.check_type_of_vehicle, True, order["Type"])
		self.check_if_valid(
			"license plate", 
			self.order_manager.check_license_plate, 
			True, 
			order["License plate"]
		)
		self.check_if_valid(
			"insurance", self.order_manager.check_insurance, True, order["Insurance"])

		print()
		save_edited_order = self.nocco_list.choose_one(
			"Choose an action", 
			[
				"Save", 
				"Cancel"
	...
	def save_edited_order(self):
		"""Updates the order after it is edited by the employee. First deletes 
		the previous order, and then saves the edited order as a new order.
		"""
		self.order_manager.delete_order(self.__current_order)
		self.order_manager.save_new_order()
	...
	def handle_answer_from_customer_menus(self, prompt, menu_type):
		"""This method handles all the choices that are made when choosing
		 from the customer submenus of the application.
		"""
		####################################################    
		#                  REGISTER CUSTOMER               #                    
		####################################################
		...
		####################################################
			...
			if prompt == "Save":
				...
				self.__current_customer = self.customer_manager.find_customer_by_ssn(
					self.__current_customer.get_ssn())
				print("Customer: {}\n".format(self.__current_customer.__str__()))
				self.found_customer()
	...
	def register_customer(self):
		"""Method for registering a new customer. Provides all the required 
		checks to prevent the employee entering wrong inputs. After the 
		employee has entered all required datas, then another sub menu will appear, 
		...
		print()
		register_customer = self.nocco_list.choose_one(
			"Choose an action", 
			[
				"Save", 
				"Cancel"
		...
		self.handle_answer_from_customer_menus(
			register_customer["action"], "register customer")

	def edit_customer(self):
		"""This method is called in case that the employee wants to edit a 
		customer´s data The employee can either add new data, and it will 
		override the previous data or can leave the input field empty and 
		hit enter, then the previous data will be kept unchanged.
		"""
		...
		self.check_if_valid(
			"Phone number", 
			self.customer_manager.check_phone_number, 
			True, 
			customer["Phone number"])
		self.check_if_valid(
			"Driver license category", 
			self.customer_manager.check_license, 
			True,
			customer["Driver license category"])
		self.check_if_valid(
			"Email address", 
			self.customer_manager.check_email, 
			True, 
			customer["Email address"])
		self.check_if_valid(
			"Credit card number", 
			self.customer_manager.check_credit_card, 
			True,
			customer["Credit card number"])
		self.check_if_valid(
			"Home address", 
			self.customer_manager.check_address, 
			True, 
			customer["Home address"])

		print()
		save_edited_customer = self.nocco_list.choose_one(
			"Choose an action", ["Save", "Cancel"], "action")
		self.handle_answer_from_customer_menus(
			save_edited_customer["action"], "save edited customer")
	...
	def save_edited_customer(self):
		"""When saving an edited customer, first it deletes the previous 
		incorrect customer and then saves the edited customer as new.
		"""
	...
	def find_customer(self):
		"""When find customer option is clicked, the employee will choose to find a 
		customer either by name or by SSN.
		"""
	...
	def found_customer(self):
		"""After the unique customer is found, then the employee can decide what to 
		do with that particular customer, by picking in any of the available 
		options.
		"""
	...
	def customer_history(self):
		"""If the employee wants to see the history of any particular customer this 
		method is called and it will print all the available orders made by that 
		customer, or print a message in case that the customer has never made any order.
		"""
		ssn = self.__current_customer.get_ssn()
		orders = self.order_manager.find_order_by_ssn(ssn)
		if orders == []:
			print("{}".format(self.color.return_colored(
				"No orders registered to this customer", "red")))
			self.nocco_list.single_list("Go back")
			self.frame.delete_last_lines(3)	
		else:
			...
			for order in orders:
				print("{:<10}{:<10}{:>20}".format(
					order.get_id(), order.get_license_plate(), order.get_date_str()))
			self.nocco_list.single_list("Go back")
			self.frame.delete_last_lines(len(orders) + 4)

	def find_customer_by_name(self):
		"""If the employee is looking for a customer by the name of that customer, 
		this method is called. The employee enters the customer´s name, and if no 
		customer with that particuar name exists, a short message will be printed to 
		indiciate that. In case that there are more than one customer with a particular 
		name then a message will be printed to indicate that, and the list of the 
		cusomers will be printed in the screen for the employee to choose from.
		In case that there is only one customer with that name, then the employee will 
		be able to edit, or delete that customer.
		"""
		...
		if customers == None: #Then there is no customer, none will go through here...
			...
			if len(customers) == 1: #If there is only one customer
				...
				print("{}".format(self.color.return_colored(
					"There are multiple customers with that name!", "red")))
				print()
				# Loops through the list of customers found to create a list for our menu
				printable_customers = [
					"{}".format(customer.__str__()) for customer 
					in customers]
				printable_customers.append("Go back")
				# Using the list in a menu for the employee to choose from
				found_multiple_customers = self.nocco_list.choose_one(
					"Choose customer", printable_customers, "customer", True)

				self.handle_answer_from_customer_menus(
					(found_multiple_customers, customers), "found multiple customers")

	def find_customer_by_ssn(self):
		"""This method is called when the employee wants to find a customer by SSN. 
		The employee enters a SSN and will either get a message that the customer does
		not exist or, if a customer is found, the customer's name will be printed along
		...
		if customer == None:
			...
			print()
			self.find_customer()
		else: #... except there can only be one customer with a particullar ssn.
			self.frame.delete_last_lines(2)
	...
	def delete_customer(self):
		...
		first the employee has to find the customer then can delete. A short 
		message will show to indicate that the customer is successfully deleted.
		"""
		self.customer_manager.delete_customer(self.__current_customer)
	...
	def handle_answer_from_vehicle_menus(self, prompt, menu_type):
		...
		####################################################
			...
			if prompt == "Save":
				...
				self.frame.delete_last_lines()
				self.__current_vehicle = self.vehicle_manager.find_vehicle_by_license_plate(
					self.vehicle_manager.get_license())
				print("Vehicle: {}\n".format(self.__current_vehicle.__str__()))
				self.found_vehicle()
				self.frame.delete_last_lines(2)
	...
	def save_new_vehicle(self):
		"""this function saves a new registered vehicle in the vehicles csv file."""
	...
	def register_vehicle(self):
		"""Here we check if a inut from the employee is valid or not. If it is not 
		valid the empoyee is informed and asked to inpt the right input. It goes 
		through check_if_valid function and check functions in vehicle_manager.
		"""
		self.frame.delete_last_lines(7)

		self.check_if_valid(
			"Vehicle type (smallcar, sedan, offroad or bus)", self.vehicle_manager.check_type)
		...
		self.check_if_valid(
			"Fuel (gasoline, diesel, electric or hybrid)", self.vehicle_manager.check_fuel)
		self.check_if_valid(
			"Driving transmission (manual or automatic)", 
			self.vehicle_manager.check_driving_transmission)
		
		print()
		register_vehicle = self.nocco_list.choose_one(
			"Choose an action", ["Save", "Cancel"], "action")
		self.handle_answer_from_vehicle_menus(register_vehicle["action"], "register vehicle")

	def find_vehicles(self):
		"""Here the employee is shown available actions after he chooses the action 
		"find vehicle".
		"""
		find_vehicles = self.nocco_list.choose_one(
			"Choose an action", 
			[
	...
	def find_vehicles_by_license_plate(self):
		"""This function handles if the user wants to find vehicles by license plate, 
		the input is compared to the data stored in vehicle_manager.
		"""
		license_plate = input("Enter license plate: ")
		print()
		vehicle = self.vehicle_manager.find_vehicle_by_license_plate(license_plate)
		# If there is no licence plate that matches the input, None will be the case.
		if vehicle == None: 
			print("{}".format(self.color.return_colored("Vehicle not found!", "red")))
			self.frame.delete_last_lines(3)
			time.sleep(1.5)
			self.find_vehicles()
		else: # No need to check if there are many licence plates because they are unique
			self.frame.delete_last_lines(2)
			...
			self.handle_answer_from_vehicle_menus(
				found_vehicles_list["action"], "found vehicle")
	...
	def find_vehicles_by_make(self):
		...
		if vehicles == None: # Similar to licence plate...
			...
			self.frame.delete_last_lines(2)
			# Here we do need to check if there is only one match or many...
			if len(vehicles) == 1: 
				...
				print("{}".format(self.color.return_colored(
					"There are multiple vehicles with that make!", "red")))
				print()
				# Creating a list of all the vehicles found
				printable_vehicles = [
					"{}".format(vehicle.__str__()) for vehicle 
					in vehicles]
				printable_vehicles.append("Go back")
				# Using the list as a menu for the employee to choose from
				found_multiple_vehicles = self.nocco_list.choose_one(
					"Choose vehicle", printable_vehicles, "vehicle", True)

				self.handle_answer_from_vehicle_menus(
					(found_multiple_vehicles, vehicles), "found multiple vehicles")
	...
	def find_vehicles_by_type(self):
		...
		if vehicles == None: #Again very similar to finding makes or licence plates
			...
			if len(vehicles) == 1: #We do need to check if there are many types
				...
				print("{}".format(self.color.return_colored(
					"There are multiple vehicles with that type!", "red")))
				print()
				printable_vehicles = [
					"{} | {}".format(
						vehicle.__str__(), 
	...
	def found_vehicle(self):
		"""If the program has found a vehicle this function will list the 
		options that the employee can choose from.
		"""
	...
	def vehicle_history(self):
		"""This functon shows the order history of a vehicle."""
		...
		if order_list == []: # The vehicle has no orders linked to it.
			print("{}".format(self.color.return_colored(
				"No orders registered to this vehicle", "red")))
			self.nocco_list.single_list("Go back")
			self.frame.delete_last_lines(3)
			...
			for order in order_list: #Loops throug the order_list
				print("{:<10}{:<15}{:>20}".format(
					order.get_id(), order.get_ssn(), order.get_date_str()))
				# Prints the id, ssn and dates for a order, using get functions in order.
			self.nocco_list.single_list("Go back")
			# Styling, lines will vary on length of order_list.
			self.frame.delete_last_lines(len(order_list) + 4) 

	def edit_vehicle(self):
		"""This function is for editing a vehicle, we need to error check the input 
		in editing like we did with registering.
		"""
		...
		self.check_if_valid(
			"Vehicle type", self.vehicle_manager.check_type, True, vehicle["Vehicle type"])
		...
		self.check_if_valid(
			"Number of seats", 
			self.vehicle_manager.check_number_of_seats, 
			True, 
			vehicle["Number of seats"])
		self.check_if_valid(
			"License plate", 
			self.vehicle_manager.check_license_plate, 
			True, 
			vehicle["License"])
		...
		save_edited_vehicle = self.nocco_list.choose_one(
			"Choose an action", ["Save", "Cancel"], "action")
		self.handle_answer_from_vehicle_menus(
			save_edited_vehicle["action"], "save edited vehicle")
		self.frame.delete_last_lines(8)
	...
	def delete_vehicle(self):
		"""If the employee wants to delete a vehicle then this method is called.
		A short message to indicate that the operation was done successfully will 
		be printed.
		"""
		self.vehicle_manager.delete_vehicle(self.__current_vehicle)
		self.frame.delete_last_lines(1)
	...
	def show_vehicle_availability(self, prompt):
		"""This method shows either all the vehicles that are available in a particular time, 
		or all that are rented, depending on the prompt that is given. The employee enters a
		start and end date and the method checks all vehicles for matching dates.
		"""
